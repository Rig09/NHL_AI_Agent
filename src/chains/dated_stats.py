...
def get_data_from_table(llm, db, sql_chain, natural_language_query, today_date):
...
def get_stats_by_dates(llm, db, sql_chain, natural_language_query, today_date):
    ...
    table = get_data_from_table(llm, db, sql_chain, natural_language_query, today_date)

    table_str = table.to_string()

    template_for_stat_description = f"""You are a tool for finding stats between given a certain date condition. Using a LangChain tool, we have fetched a table of
    ...
    You should also be able to imply some statistics from this information. For example, if someone requested a teams expected goals percentage,
    this would be the percentage of the total sum of xgoals column occuered where the shot taker was on that team.

    Corsi is another word for percentage of shots taken by a team. This is simply the proportion of rows where the team listed took the shot.
    
    For addition context, todays date is {today_date}
    """
